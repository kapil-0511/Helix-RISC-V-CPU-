`timescale 1ns/1ps
`include "defines.v"
// tb_gcd.sv — GCD via Euclidean subtraction, BL/BX subroutine, 6 pairs
...
module tb_gcd;
    ...
    task automatic load_program();
        ...
        apb_write(addr, enc_i(`FI_MOVI, 4'd7, 4'd0, 16'd6));   addr += 4;
        // loop top (word 35)
        ...
        // addr = 0x0B0 = word 44 — GCD subroutine
        ...
        apb_write(addr, enc_r(`FR_SUB, 4'd1, 4'd1, 4'd0));     addr += 4; // SUB R1,R1,R0
        apb_write(addr, enc_b(`COND_AL, 25'h1FFFFFC));          addr += 4; // B AL -4 → word 45
        // word 50 (BGT target)
        apb_write(addr, enc_r(`FR_SUB, 4'd0, 4'd0, 4'd1));     addr += 4; // SUB R0,R0,R1
    ...
    initial begin
        ...
        $display("[TB]  rst_n=1 — CPU starting  (CPI=5, timeout=5000 cycles)");

        fork
            begin
                repeat(5000) @(posedge clk);
                $display("TIMEOUT: 5000 cycles");
                $stop;
            end
            begin
