`timescale 1ns/1ps
`include "defines.v"
// tb_minmax.sv — Find min and max of N=25 elements
...
module tb_minmax;
    ...
    task automatic load_program();
        int addr;
        $display("[APB] Loading min/max program (N=25)...");
        ...
        apb_write(addr, enc_load(4'd3, 4'd0, 16'd0));             addr += 4; // LDR  R3,[R0,0] min=A[0]
        apb_write(addr, enc_load(4'd4, 4'd0, 16'd0));             addr += 4; // LDR  R4,[R0,0] max=A[0]
    ...
    task automatic load_array();
        ...
        $display("[TB]  Array[0..24] loaded. Expected min=18, max=951");
    endtask
    ...
    initial begin
        ...
        $display("[TB]  rst_n=1 — CPU starting  (CPI=5, timeout=3500 cycles)");

        fork
            begin
                repeat(3500) @(posedge clk);
                $display("TIMEOUT: 3500 cycles");
                $stop;
            end
            begin
